...
local scissors = stats.scissors
local powerstaff = stats.powerstaff
local axe_actions = {}
local scissors_actions = {}
local fast_pick = nil
...
----------------------------------------------------------------------------
do
    local prefab_id = scissors.prefab_id
    local hammer_mode_tag = scissors.hammer_mode_tag

    if config.scissors then
        -- 右键切换分解/锤子形态
        if config.hammer then
            scissors_actions.toggle_mode = core.make_toggle_action('YMKIT_SCISSORS_TOGGLE_MODE', '切换为锤子形态', {
                prefab_id = prefab_id,
                setter = 'SetMode',
        ...
        if config.scissors_remote then
        ...
        local dismantle_fast = {
            name = 'ymkit_scissors_fast_dismantle',
            animation = 'atk',
            pretag = 'predismantle',
            worktag = 'dismantling',
            work_frame = 2,
        ...
        local function can_queue_dismantle(target)
            ...
            if active ~= nil and active.prefab == prefab_id then
                scissors_inst = active
            elseif equipped ~= nil and equipped.prefab == prefab_id then
                scissors_inst = equipped
            end
            if scissors_inst == nil or scissors_inst:HasTag(hammer_mode_tag) then
                return false
            end
        ...
        -- 隔空锤子
        scissors_actions.remote_tools = {}
        for _, definition in ipairs(scissors.tools) do
            if config.scissors_remote and definition.remote and config[definition.config] then
                local entry = core.create_remote_tool(definition, {
                    prefix = 'YMKIT_SCISSORS_REMOTE_',
                    prefab_id = prefab_id,
                    require_tag = hammer_mode_tag,
                })
                table.insert(scissors_actions.remote_tools, entry)
            end
        end

        -- 使用剪刀动作时不自动装备到手上，方便连续使用
        local no_autoequip_actions = {[ACTIONS.YMKIT_DISMANTLE] = true}
        if #scissors_actions.remote_tools > 0 then
            for _, entry in ipairs(scissors_actions.remote_tools) do
                core.register_remote_tool(entry)
...
AddComponentAction('INVENTORY', 'equippable', function(inst, doer, actions)
    ...
    elseif scissors_actions.toggle_mode ~= nil
        and inst.prefab == scissors.prefab_id
        and core.is_equipped_by(inst, doer, scissors.prefab_id) then
        table.insert(actions, scissors_actions.toggle_mode)
    end
end)
...
AddComponentAction('EQUIPPED', 'tool', function(inst, doer, target, actions, right)
    ...
    elseif inst.prefab == scissors.prefab_id then
        for _, entry in ipairs(scissors_actions.remote_tools) do
